...
def send_message(author_uid, message_content, target_gid, **kwargs):
    if set(['tl_id','sl_id']).issubset(kwargs):
        message = Messages(author_uid,message_content,tl_id=kwargs.get('tl_id'),sl_id=kwargs.get('sl_id'))
    else:
